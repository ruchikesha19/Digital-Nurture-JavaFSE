...
### 2. Java Projects (Gradle)
To inspect Gradle projects:
1. Apply the SonarQube plugin in your `build.gradle` file:
   ```groovy
   plugins {
       id "org.sonarqube" version "4.4.1.3373"
   }
   ```
2. Configure Sonar properties in `build.gradle`:
   ```groovy
   sonar {
       properties {
           property "sonar.projectKey", "my-gradle-project"
           property "sonar.projectName", "My Gradle Project"
           property "sonar.host.url", "http://localhost:9000"
           property "sonar.token", "YOUR_GENERATED_SONAR_TOKEN"
       }
   }
   ```
3. Run the scan:
...
### 3. .NET Projects
For .NET Core, .NET 5+, or .NET Framework projects, you need to use the `dotnet-sonarscanner` CLI tool.

1. Install the tool globally:
   ```bash
   dotnet tool install --global dotnet-sonarscanner
   ```
2. Run the scanner commands in sequence from your project root:
   ```bash
   dotnet sonarscanner begin /k:"my-dotnet-project" /d:sonar.host.url="http://localhost:9000" /d:sonar.token="YOUR_GENERATED_SONAR_TOKEN"
   dotnet build
   dotnet sonarscanner end /d:sonar.token="YOUR_GENERATED_SONAR_TOKEN"
   ```
...
1. Install the `sonar-scanner` CLI tool globally, or download the binary.
2. In your Python project root, create a file named `sonar-project.properties`:
   ```properties
   ...
   ```
3. Run the scanner command in your python directory:
   ```bash
   sonar-scanner
   ```
